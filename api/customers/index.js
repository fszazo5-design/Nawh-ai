...
export default async function handler(req) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (search) {
        return jsonResponse(await sql`SELECT * FROM customers WHERE name ILIKE ${'%' + search + '%'} OR phone ILIKE ${'%' + search + '%'} ORDER BY created_at DESC`);
      }
      return jsonResponse(await sql`SELECT * FROM customers ORDER BY created_at DESC`);
    }
